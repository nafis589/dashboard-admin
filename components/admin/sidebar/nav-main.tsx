...
import { VENDORS_URL, type NavGroup, type NavMainItem } from "@/config/admin-navigation";
...
const NavItemExpanded = ({
  ...
  const showBadge = item.url === VENDORS_URL && item.badge && pendingCount > 0;
...
export function NavMain({ items }: NavMainProps) {
  ...
  const { data, refetch } = useAdminStats();
  const count = data?.data?.pending_vendors ?? 0;
            ...
            <SidebarMenu>
              {group.items.map((item) => {
                if (state === "collapsed" && !isMobile) {
                  if (!item.subItems) {
                    const showBadge = item.url === VENDORS_URL && item.badge && count > 0;
                    return (
                      <SidebarMenuItem key={item.title}>
                        <SidebarMenuButton
                          asChild
